package SysAdmToolkit::Lvm::PhysicalVolume::HPUX_1111;

=head1 NAME

SysAdmToolkit::Lvm::PhysicalVolume::HPUX_1111 - OS and version specific module for getting info about physical volume

=head1 SYNOPSIS

	my $pv= SysAdmToolkit::Lvm::PhysicalVolume::HPUX_1111->new('pv' => '/dev/dsk/c5t2d3');
...
sub _init() {
	...
	my $pvdisplayCmd = "$pvCmd $pvPath |grep -v 'Alternate Link'|grep -v '\\-\\-\\-'|awk '{print \$NF}'";
...
=head1
we don't need this method, if PV is not in VG we cannot get actual info, so it is destroyed
...
sub pvRemove() {
	...
	if(!$params{'pvname'}) {
		die "You need to provide block device file of disk!\n";
	} else {
		
		$pvRemoveInfo = $shell->execCmd('cmd' => "$pvRemoveCmd " . $params{'pvname'}, 'cmdsNeeded' => [$pvRemoveCmd]);
		
		if(!$pvRemoveInfo->{'returnCode'}) {
			warn("There was some problem while removing PV " . $pvRemoveInfo->{'msg'} . "\n");
		} # if
		
	} # if
	
	return $pvRemoveInfo->{'returnCode'};
