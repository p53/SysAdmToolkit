package SysAdmToolkit::Storage::Disk::HPUX_1131;

=head1 NAME

SysAdmToolkit::Storage::Disk::HPUX_1131 - this is HP-UX 11.31 specific module for getting info about disk/lun

=head1 SYNOPSIS

	my $mydisk = SysAdmToolkit::Storage::Disk::HPUX_1131->new('hwPath' => '4/0/1/3/2.1.2');
	my $mydisksecond = SysAdmToolkit::Storage::Disk::HPUX_1131->new('devFile' => '/dev/dsk/cdisk12');
	$mydisk->{'driver'};
	$mydisksecond->{'hwPath'};
...
params:
	...
	hwPath - string or instance - number or devFile - string or lunpath - string
...
sub _init() {
	...
	} elsif($params{'lunpath'}) {
		my $lunpathInfo = $shell->execCmd('cmd' => "$scan -m hwpath -H $params{'lunpath'}|grep 64000", 'cmdsNeeded' => [$scanCmd]);
		$identifier = $lunpathInfo->{'msg'};
		$options = '-H';
	} else {
		die "There must be just one parameter passed: hwPath, instance or devFile!\n";
	} # if
	
	my $generalInfo = $shell->execCmd('cmd' => "$scan -AeF $options $identifier", 'cmdsNeeded' => [$scanCmd]);
	my $devFileInfo = $shell->execCmd('cmd' => "$scan -fn $options $identifier", 'cmdsNeeded' => [$scanCmd]);
