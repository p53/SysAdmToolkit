package SysAdmToolkit::Storage::Lun::Admin::RedHat_65;

=head1 NAME

SysAdmToolkit::Storage::Lun::Admin::RedHat_65 - interface module for getting info about Luns

=head1 SYNOPSIS

	my $lunAdmin = SysAdmToolkit::Storage::Lun::Admin::RedHat_65->new();
...
my %arrayTypesCmd = (
						'eva' => {'cmd' => 'evainfo', 'options' => '-al'},
						'hitachi' => {'cmd' => 'inqraid', 'options' => '-CLIWP -fcx /dev/rdsk/*'},
						'emc' => {'cmd' => 'inq.LinuxAMD64' , 'options' => '-showvol -nodots -sid -f_emc|grep -v "Inquiry utility"|grep -v "For help"|grep -v "Copyright"|grep -v ":SER NUM"|grep -v "\---------------"|grep -v ^$'},
					);
...
sub getLuns() {
	...
	# getting command for selected vendor
...
sub checkPathCount() {
	...
	# for each lun check how many paths are there and compare against count value
	for my $lun(@$luns) {
...
sub findBy() {
	...
	for my $record(@$filteredRecords) {
		...
		# supplied parameter by which to filter must be present in order array
		for my $filter(@$order) {
...
sub findOneBy() {
	...
	for my $record(@$filteredRecords) {
		...
		if($pass == 1) {
			...
			# if there is already record with supplied filter, don't add new one
...
sub parseEva() {
	...
	for my $evaLun(@$evaLuns) {
		...
		# filter out initial lines and parse lines
