package SysAdmToolkit::Term::Shell;

=head1 NAME

	SysAdmToolkit::Term::Shell - module for executing shell commands
...
sub analyzeResult($$$) {
	...
	if ($code == -1) {
		$returnCode = 0;
		$msg = "Command $fullCmd failed to execute: $!\n";
	} elsif($code == 127) {
		$returnCode = 0;
	...
	$result = {'returnCode' => $returnCode, 'msg' => $msg};
...
Method warning serves for creating warning if return code isn't 1 - this uses module internal return codes, not shell!
...
Method fatal serves for dieing if return code of command isn't 1 - this uses module internal return codes, not shell!
